...
public class Test {
	...
	private static final File dbPath = new File( "/Users/Ximena.DESKTOP-556H0D0/Documents/ProgramLanguages/neo4j-community-3.0.7/data/databases/graph.db");

	private GraphDatabaseService graphDb;
	static ArrayList<ListArt> ListNode = 	new ArrayList<ListArt>();
	static ArrayList<ListArt> ListPrincipal = 	new ArrayList<ListArt>();
	...
	public static void main(String[] args) throws IOException
	{
		...
		wikipedia.setUp();
		wikipedia.PrintWikiArticles(); 	   // find the all nodes in the component (fill the listPrincipal)
		for(int i=0;i<ListPrincipal.size();i++){
			wikipedia.FindoneNode(ListPrincipal, i);      // find a node depending of wikiid //Fill the ListNode
			System.out.println(wikipedia.printNodeFriends(ListNode));//Detect the incorrect ones
		}
	...
	public static ArrayList<ListArt> visit(GraphDatabaseService graphDb, Node start,/* Set<Long> visited,*/ int NumberCross, ListArt aux2, ArrayList<ListArt> listNode2) { 
		...
		for (Node nn : graphDb.traversalDescription().depthFirst().relationships(Rels.crosslink, Direction.BOTH).traverse(start).nodes()) { 
			...
			ListPrincipal.add(aux2);
			System.out.println(  "Article => "+nn.getProperty("wikiid") + " -- " + nn.getProperty("lang")+ "---");
	...
	private ArrayList<ListArt> FindoneNode(ArrayList<ListArt> listPrincipal2, int i) 
	{
		//----- Declarar Variables----------//
		final String Art = "Article";
		int NumberCross = 0;
		...
		try (Transaction tx = graphDb.beginTx()) 
		{
			FindPrincipal = listPrincipal2.get(i).getWikiid();
			FindLangP= listPrincipal2.get(i).getLang();
/*el*/		ResourceIterator<Node> it = graphDb.findNodes(recordClassLabel, "wikiid", FindPrincipal);
/*572*/			Node next = null; 
/*en*/			while( it.hasNext() ) { 
/*12630*/		next = it.next(); 
/*en*/			String lang = (String)next.getProperty("lang");
				if ( lang.equals(FindLangP) ) 
					break; 
...
//		double ValidPerc = 0.50;
//		double ValidPerc = 0.40;
...
//						}
					...
					if(singleton == 0){
						ListIncorrects.add(listNode2.get(0).getWikiid()+"-"+listNode2.get(0).getLang()+"--"+
										   (String) start.getProperty("wikiid")+"-"+(String) start.getProperty("lang")); // add to ListIncorrects the nodes
						break;
					}
				}
			}
//--------------------------------------------------------------------------
			if (listNode2.size() >= 3){
				for( Node friend : traverser.nodes() )
				{
					...
					if(redi == 0){
						ListIncorrects.add(listNode2.get(0).getWikiid()+"-"+listNode2.get(0).getLang()+"--"+
											(String) start.getProperty("wikiid")+"-"+(String) start.getProperty("lang"));
					}
